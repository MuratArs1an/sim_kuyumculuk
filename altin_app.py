...
def gold_prices():
    ...
    try:
        ...
        data.append({"urun": "Gram Altın (24 Ayar)",
                     "alis": f"{gram_alis * 0.995-50:,.2f}",
                     "satis": f"{gram_satis * 1.008 + 200:,.2f}"})
        ...
        data.append(sar_item("stl2",  "E. Çeyrek", satis_ek=200))
        data.append(sar_item("stl4",  "E. Yarım",  satis_ek=400))
        data.append(sar_item("stl6",  "E. Teklik", satis_ek=800))

        # Ata Lira: senin mantığını korudum (alış -50, satış +200)
        ata = sar_item("stl10", "E. Ata Lira", satis_ek=1000, alis_ek=-100)
        data.append(ata)

        data.append({"urun": "22 Ayar Hurda Bilezik",
                     "alis": f"{gram_alis * 0.912 -20 :,.2f}",
                     "satis": f"{gram_satis * 0.912 + 50:,.2f}"})
